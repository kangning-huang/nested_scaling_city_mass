...
library(pacman)
pacman::p_load(Matrix,sf,lme4,readr,dplyr,performance,tibble,ggplot2,rprojroot,scales,viridis,cowplot,tidyr,ggspatial,moments,poweRlaw)
```

This notebook analyzes neighborhood-level scaling patterns using H3 hexagonal grids to examine how material stocks scale with population at the sub-city level.
...
mass_avg_data <- readr::read_csv(
  # file.path("..", "results", "global_scaling", "mass_avg_Li2022_vs_Liu2024_H3_grids_world.csv")
...
data <- readr::read_csv(
  # file.path("..", "results", "global_scaling", "mass_avg_Li2022_vs_Liu2024_H3_grids_world.csv")
  file.path("..", "results", "global_scaling", "Fig3_Mass_Neighborhood_H3_Resolution6_2025-06-24.csv")
  ) %>%
  dplyr::rename(
    mass_building = BuildingMass_AverageTotal,
    mass_mobility = mobility_mass_tons,
...
city_slope %>% write.csv(
  file.path("..", "results","global_scaling","city_slope.csv"))
```
...
# Read all cities
sf_cities <- sf::read_sf(file.path("..","results","shapefiles","all_cities_h3_grids.gpkg")) %>%
...
country_slope <- read.csv(
  file.path("..", "results","global_scaling","country_inset_slope.csv")) %>%
...
city_data_export %>% write_csv(file.path("..", "results","global_scaling","city_inset_slope_2025-07-05.csv"))
